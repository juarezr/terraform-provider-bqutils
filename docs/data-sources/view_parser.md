...
### Applying access permissions to views by using BigQuery authorized views

Cross-dataset views often need [authorized view](https://cloud.google.com/bigquery/docs/authorized-views) grants on the source datasets. Use `dataset_references` with `for_each` on [google_bigquery_dataset_access](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/bigquery_dataset_access). Unlike routines, view `query` bodies are not rewritten (the Tables API does not require Routines-style project qualification).
...
- `dataset_references` (List of String) Distinct dataset IDs referenced in the view query that differ from the view's own dataset. Empty when the query only uses the home dataset. Useful with google_bigquery_dataset_access for authorized views.
